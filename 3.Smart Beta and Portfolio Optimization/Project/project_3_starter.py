...
def generate_dollar_volume_weights(close, volume):
    ...
    dollar_volume_weights = pd.DataFrame(columns=close.columns, index=close.index, dtype=np.float64)
    
    #product = close * volume
    #dollar_volume_weights = product.div(product.sum(),axis=0).astype(float)
    
    for i in range(dollar_volume_weights.shape[0]):
        product = close.iloc[i,:] * volume.iloc[i,:]
        dollar_volume_weights.iloc[i,:] = (product / product.sum()).astype(float)
...
def calculate_dividend_weights(dividends):
    ...
    """
    #TODO: Implement function
    dividend_weights = pd.DataFrame(index=dividends.index, columns=dividends.columns,dtype=np.float64)
    total = np.cumsum(dividends.sum(axis=1))
    
   # dividend_weights = dividends.shift(1).sum() / total
    
    for i in range(dividends.shape[0]):
        dividend_weights.iloc[i,:] = dividends.iloc[:i+1,:].sum() / total[i]
...
def calculate_cumulative_returns(returns):
    ...
    #cumulative_returns = returns.groupby('date',axis =0).sum()
    
    return np.cumprod(returns.sum(axis=1)+1)
...
def rebalance_portfolio(returns, index_weights, shift_size, chunk_size):
    ...
    rebalance_indices = []
    for rebalance_index in range(chunk_size, len(returns), shift_size):
        chunk = returns.iloc[rebalance_index - chunk_size:rebalance_index]
        rebalance_indices.append(rebalance_index)
        weights = index_weights.iloc[rebalance_index-1]
        cov = get_covariance_returns(chunk)
        optimal_weights = get_optimal_weights(cov, weights)
...
def get_portfolio_turnover(all_rebalance_weights, shift_size, rebalance_count, n_trading_days_in_year=252):
    ...
    for i, weight in enumerate(all_rebalance_weights[:-1]):
        sum_total_turnover += np.sum(np.abs(weight - all_rebalance_weights[i+1]))
